...
public class LevelScreen : MonoBehaviour
{
    public Button[] levelButton; // Array to hold references to the level buttons
    public Sprite lockSprit; // Reference to the lock sprite
    public Sprite unlockedSprit; // Reference to the unlocked level sprite or default sprite
    ...
    void Start()
    {
        ...
        for (int i = 0; i < levelButton.Length; i++)
        {
            ...
            {
                // Level is locked
                levelButton[i].image.sprite = lockSprit; // Display the lock sprite
                levelButton[i].interactable = false; // Disable the button
            }
            else
            {
                // Level is unlocked
                levelButton[i].image.sprite = unlockedSprit; // Display the unlocked sprite or leave as default
                levelButton[i].interactable = true; // Enable the button
                int levelIndex = i + 1;
                levelButton[i].onClick.AddListener(() => LoadLevel(levelIndex)); // Add listener to load the level
            }
        }
    }
